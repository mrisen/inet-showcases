...
Block Acknowledgment is a 802.11 MAC feature, which increases throughput by decreasing
...
Block acknowledgement frames (block acks or BAs) are used within block ack sessions.
...
is for one direction only, e.g. a separate block ack session must be established for
...
- ``MixedTraffic``: Two traffic sources in ``host1`` generate packets below and above the :par:`maxBlockAckPolicyFrameLength` threshold. Packets with size below the threshold are block acked, those below aren't.
